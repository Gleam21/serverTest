...
router.get('/info', function(req, res, next) {
  ...
  if (userId) {
    ...
    usersCollection.findOne({_id: mongodb.ObjectID(userId)},{projection: {name: true, score: true}},
      function(err, result) {
...
router.post('/addscore', function(req, res, next) {
  ...
  var scorevalidate = scoreValidation(score);
  if (scorevalidate == false) {
    res.status(400).json({message:'10점이상 한번에 추가할 수 없습니다.'});
    return;
  }
...
var scoreValidation = function(score) {
